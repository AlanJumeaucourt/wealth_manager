...
def process_request(required_fields: Optional[list[str]] = None, type_of_request: Literal['GET', 'POST', 'PUT', 'DELETE'] = 'GET') -> tuple[int, Optional[dict[str, Any]], Optional[Response], Optional[int]]:
    """
    ...
    if required_fields:
        if not data:
            return user_id, None, jsonify({"error": f"Missing required fields: {', '.join(required_fields)}"}), 400
        
        not_recognized_fields = [field for field in data.keys() if field not in required_fields]
        missing_fields = [field for field in required_fields if field not in data]
        at_least_one_field = len(required_fields) == len(missing_fields) and len(missing_fields) > 0
        
        if type_of_request == 'PUT':
            if at_least_one_field:
                return user_id, None, jsonify({"error": f"Missing at least one field: {', '.join(required_fields)}"}), 400

        elif type_of_request == 'POST':
            if not_recognized_fields:
                return user_id, None, jsonify({"error": f"Not recognized fields: {', '.join(not_recognized_fields)}"}), 400
            if missing_fields:
                return user_id, None, jsonify({"error": f"Missing required fields: {', '.join(missing_fields)}"}), 400
