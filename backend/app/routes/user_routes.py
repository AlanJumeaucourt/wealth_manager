...
def update_user_route(user_id: int):
    user_id_from_tokens, data, error, code = process_request(['name', 'email', 'password'], type_of_request='PUT')
    ...
    user = update_user(user_id, validated_data.get('name'), validated_data.get('email'), validated_data.get('password'))
    if user:
