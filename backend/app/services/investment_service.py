...
class InvestmentService(BaseService):
        ...
        try:
            ...
            for position in positions:
                try:
                    ...
                    print(
                        f"Error fetching current price for {position['asset_symbol']}: {e}"
                    )
    ...
    def get_asset_transactions(
        ...
        try:
            ...
            result = {"buys": [], "sells": [], "deposits": [], "withdrawals": []}
                    ...
                    result[activity_list].append(
                        {
                            ...
                            "total_paid": (
                                float(trans["total_paid"]) if trans["total_paid"] else 0
                            ),
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    logger.info(
                        f"historical_data for {symbol}: {historical_data[:2]}..."
                    )  # Log first 2 entries
            ...
            for date in all_dates:
                ...
                if daily_value > 0:  # Only add points where we have a value
                    performance_data.append(
                        {"date": date, "cumulative_value": round(daily_value, 2)}
                    )
