...
class StockService:
            ...
            if "quotes" in search_data:
                for quote in search_data["quotes"]:
                    ...
                    try:
                        ...
                        print(
                            f"Error fetching additional info for {quote.get('symbol')}: {str(e)}"
                        )
    ...
    def get_historical_prices(
        ...
        if period not in valid_periods:
            raise ValueError(
                f"Invalid period: {period}. Please use one of the following: {', '.join(valid_periods)}"
            )
