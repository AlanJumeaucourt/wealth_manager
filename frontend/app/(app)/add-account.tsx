...
export default function AddAccountScreen() {
    ...
    try {
      if (account) {
        await updateAccount(account.id, {
          name: AccountName,
          type: AccountType as import("@/types/account").Account["type"],
          bank_id: bankId,
          currency: AccountCurrency,
          balance:
            typeof (account as { balance?: number }).balance === "number"
  ...
  const options = ["USD", "EUR", "RON", "GBP", "JPY", "CAD"];
  ...
  const renderPicker = (
    label: string,
    value: string,
    pickerWidth: number = 100,
    onPress: (() => void) | ((itemValue: string) => void),
  ) => {
    ...
    if (Platform.OS === "ios") {
      return (
        <TouchableOpacity onPress={onPress as () => void} style={pickerStyle.pickerButton}>
          <Text style={pickerStyle.pickerLabel}>{value ? value : `Select ${label}`}</Text>
        </TouchableOpacity>
      );
    } else {
      return (
        <View>
          <Picker
            selectedValue={value}
            onValueChange={onPress as (itemValue: string) => void}
  ...
  const handleDeleteBank = async (bankId: number) => {
    try {
      ...
      void dispatch(fetchBanks());
      setFormData((prev) => ({ ...prev, AccountBankId: "" }));
    } catch (error) {
  ...
  const handleAddBank = async () => {
    if (!formData.newBankName) {
      console.error("Please enter a bank name");
      return;
    }

    try {
      await createBank(formData.newBankName);
      void dispatch(fetchBanks());
      handleChange("newBankName", "");
      setIsAddingNewBank(false);
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          ...
          {renderPicker(
            ...
            Platform.OS === "ios"
              ...
              : (itemValue: string) => handleChange("AccountType", itemValue),
          )}
            ...
            <View style={styles.bankSelectionContainer}>
              {renderPicker(
                ...
                formData.AccountBankId
                  ? banks.find((bank: Bank) => bank.id === formData.AccountBankId)?.name
                  : "",
                isAddingNewBank ? 85 : 100,
                Platform.OS === "ios"
                  ? showBankPicker
                  : (itemValue: string) => handleBankSelection(itemValue),
              )}
          ...
          {renderPicker(
            ...
            Platform.OS === "ios"
              ? showCurrencyPicker
              : (itemValue: string) => handleChange("AccountCurrency", itemValue),
          )}
