...
import type { StockHistoryPoint, StockInfo } from "@/types/stock";
    ...
    >
      <View style={styles.transactionHeader}>
        <View style={styles.transactionTypeContainer}>
          <View style={[styles.transactionTypeTag, isBuy ? styles.buyIcon : styles.sellIcon]}>
            <Ionicons
          ...
          <View style={styles.transactionInfo}>
            <View style={styles.transactionMainInfo}>
              ...
              <Text style={styles.priceText}>
                @ {formatCurrency(transaction.unit_price, currencyCode)}
              </Text>
            </View>
        ...
        <View style={styles.transactionTotalContainer}>
          <Text style={styles.feeText}>Fee: {formatCurrency(transaction.fee, currencyCode)}</Text>
          <Text style={styles.totalValueText}>
            {formatCurrency(isBuy ? -total : total, currencyCode)}
          </Text>
        </View>
      </View>
    </Pressable>
...
export const StockDetails: React.FC = () => {
  ...
  const [historicalPrices, setHistoricalPrices] = useState<StockHistoryPoint[]>([]);
  ...
  const stockCurrency = stockInfo?.info?.currency || "EUR";
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const buys = txs.buys;
        const sells = txs.sells;

        if (stockInfoData?.info?.longName) {
          setStockName(stockInfoData.info.longName);
        }

        const pricesArray: StockHistoryPoint[] = Array.isArray(pricesData) ? pricesData : [];

        if (buys.length > 0) {
          const firstBuyDate = buys.reduce(
            (minDate: string, current: AssetTransaction) =>
              current.date < minDate ? current.date : minDate,
            buys[0]!.date,
          );
          const filteredPrices = pricesArray.filter((price) => price.date >= firstBuyDate);
  ...
  const handleTransactionPress = (transaction: AssetTransaction, isBuy: boolean) => {
    router.push({
      pathname: "/(app)/add-investment-transaction",
      params: {
  ...
  const formatChartData = () => {
    ...
    transactions.sells?.forEach((sell) => {
      transactionPoints.set(sell.date, {
        ...
        customDataPoint: () => (
          <View
            style={[styles.transactionPoint, { backgroundColor: darkTheme.colors.error }]}
          ></View>
        ),
      });
    });
    ...
    return {
      ...
      secondaryData: secondaryData.sort(
  ...
  const StockMetrics = ({ info }: { info: NonNullable<StockInfo["info"]> }) => (
    <View style={styles.metricsContainer}>
      ...
      <View style={styles.metricRow}>
        ...
        <View style={styles.metric}>
          ...
          <Text style={styles.metricValue}>
            {info.regularMarketVolume != null ? info.regularMarketVolume.toLocaleString() : "—"}
          </Text>
        </View>
      </View>
      <View style={styles.metricRow}>
        <View style={styles.metric}>
          ...
          <Text
            style={[
              ...
              {
                color:
                  (info.ytdReturn ?? 0) >= 0 ? darkTheme.colors.success : darkTheme.colors.error,
              },
            ]}
          >
            {info.ytdReturn != null ? `${(info.ytdReturn * 100).toFixed(2)}%` : "—"}
          </Text>
        </View>
        <View style={styles.metric}>
  ...
  const SectorWeightings = ({
    ...
    sectors: NonNullable<StockInfo["fund_sector_weightings"]>;
  }) => (
        ...
        >
            ...
            {chartData.mainData.length > 0 ? (
              <View style={styles.chartContainer}>
                <LineChart
                  ...
                  yAxisLabelSuffix={` ${stockCurrency}`}
                  yAxisLabelPrefix=""
                  rulesType="solid"
                  xAxisThickness={0}
                      ...
                      return (
                        <View style={styles.tooltipContainer}>
                          <Text style={styles.tooltipValue}>
                            {formatCurrency(item.value, stockCurrency)}
                          </Text>
                ...
                <View style={styles.legendContainer}>
                  <View style={styles.legendItem}>
                    <View
                      style={[styles.legendDot, { backgroundColor: darkTheme.colors.primary }]}
                    />
                    <Text style={styles.legendText}>Stock Price</Text>
                  </View>
                  <View style={styles.legendItem}>
                    <View
                      style={[styles.legendDot, { backgroundColor: darkTheme.colors.success }]}
                    />
          ...
          {stockInfo?.info && (
            <View style={styles.section}>
              ...
              <StockMetrics info={stockInfo.info as NonNullable<StockInfo["info"]>} />
            </View>
          )}
