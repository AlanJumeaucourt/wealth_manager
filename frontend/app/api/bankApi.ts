...
export const updateAccount = async (
  accountId: number,
  accountData: Pick<Account, "name" | "type" | "bank_id" | "currency" | "balance">,
) => {
  try {
...
export const fetchWealthData = async (startDate: string, endDate: string) => {
  try {
    const response = await apiClient.get(
      `/accounts/balance_over_time?start_date=${startDate}&end_date=${endDate}`,
    );
...
export const fetchBudgetSummary = async (
  ...
  try {
    const response = await apiClient.get(
      `/budgets/summary?start_date=${startDate}&end_date=${endDate}`,
    );
...
export const updatePreferredCurrency = async (preferred_currency: string) => {
  try {
    const response = await apiClient.put("/users/preferred_currency", { preferred_currency });
    return response.data;
  } catch (error) {
    return handleApiError(error, "Error updating preferred currency");
  }
};
...
export const fetchInvestmentTransactions = async (
  ...
  try {
    const response = await apiClient.get(
      `/investments?per_page=${perPage}&page=${page}${accountId ? `&account_id=${accountId}` : ""}`,
    );
...
export const getStockHistory = async (symbol: string, period: string = "1y") => {
  try {
    const response = await apiClient.get(
      `/stocks/${encodeURIComponent(symbol)}/history?period=${period}`,
    );
    return response.data;
  } catch (error) {
...
export const getCurrentHistory = async (symbol: string) => {
  try {
    const response = await apiClient.get(
      `/stocks/${encodeURIComponent(symbol)}/history?period=max`,
    );
