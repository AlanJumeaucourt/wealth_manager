...
export const getInstitution = async (institutionId: string): Promise<GoCardlessInstitution> => {
  const token = localStorage.getItem("access_token");

  try {
    const response = await fetch(`${GOCARDLESS_API_URL}/institutions/${institutionId}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
      }
    });
    ...
    if (!response.ok) {
      throw new Error(`Failed to get institution details: ${response.statusText}`);
    }
...
export const createEndUserAgreement = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.summary || `Failed to create end user agreement: ${response.statusText}`);
    }
...
export const getRequisitionByReference = async (reference: string): Promise<GoCardlessRequisition> => {
  const token = localStorage.getItem("access_token");
...
export const getAccountsByRequisition = async (requisitionId: string): Promise<GoCardlessAccount[]> => {
  const token = localStorage.getItem("access_token");
...
export const getAccountDetails = async (accountId: string, updateCache: boolean = false): Promise<GoCardlessAccountDetail> => {
  const token = localStorage.getItem("access_token");

  try {
    const response = await fetch(`${GOCARDLESS_API_URL}/accounts/${accountId}/details?update_cache=${updateCache}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
      }
    });
...
export const getAccountBalances = async (accountId: string, updateCache: boolean = false): Promise<GoCardlessAccountBalance> => {
  const token = localStorage.getItem("access_token");

  try {
    const response = await fetch(`${GOCARDLESS_API_URL}/accounts/${accountId}/balances?update_cache=${updateCache}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
      }
    });
...
export const handleGoCardlessCallback = async (
  ...
  try {
    ...
    if (requisition.status !== 'LN' && requisition.status !== 'GA') {
      throw new Error(`Requisition is not in a completed state. Current status: ${requisition.status}`);
    }
    ...
    await linkAccountsToUser(requisitionId, accounts.map(acc => acc.id));
