...
export function CommandPalette() {
  ...
  const { data: assetsData, isLoading: assetsLoading } = useAssets(searchListParams, {
    enabled: shouldSearch,
  });
  ...
  const { data: liabilitiesData, isLoading: liabilitiesLoading } = useLiabilities(
    shouldSearch ? ({ ...searchListParams } as LiabilityFilters) : undefined,
    { enabled: shouldSearch },
  );
  ...
  const filteredPages = search
    ? staticPages.filter((page) => page.name.toLowerCase().includes(search.toLowerCase()))
    : staticPages;
