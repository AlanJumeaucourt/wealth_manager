...
export function AccountBalanceChart({ currentBalance, balanceHistory }: Props) {
  ...
  const {
    ...
    if (!balanceHistory || !Array.isArray(balanceHistory)) return {
      filteredData: [],
    ...
    const referenceDate = new Date(balanceHistory[balanceHistory.length - 1].date)
    ...
    const valueChange = currentBalance - (filteredData[0]?.value ?? currentBalance)
      ...
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        ...
        <Select value={timeRange} onValueChange={setTimeRange}>
          ...
          <SelectContent className="rounded-xl">
            ...
            <SelectItem value="max" className="rounded-lg">Max</SelectItem>
          </SelectContent>
        </Select>
      </CardHeader>
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        <div className="h-[250px] w-full">
          <ResponsiveContainer width="99%" height="100%" style={{ overflowY: "hidden" }}>
            <AreaChart
              data={filteredData}
