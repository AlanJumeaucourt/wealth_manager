...
export function RecordInitialPaymentsDialog({
  isOpen,
  onClose,
  liability,
}: RecordInitialPaymentsDialogProps) {
  ...
  const allFetchedTransactions = useMemo(
  ...
  const { data: accountData } = useAccounts({
    id: liability.account_id ? [liability.account_id] : [],
  });
  ...
  const onSubmit = (data: PaymentFormData) => {
    ...
    recordPaymentMutation.mutate(
      paymentPayload as Omit<LiabilityPayment, "id" | "created_at" | "updated_at">,
      {
        onSuccess: () => {
          toast({
  ...
  const handleRecordAllDisplayed = async () => {
    ...
    const promises = displayedTransactions.map((transaction) => {
      ...
      return recordPaymentMutation
        .mutateAsync(paymentPayload as Omit<LiabilityPayment, "id" | "created_at" | "updated_at">)
    ...
    if (successCount > 0) {
      toast({
        ...
        description: `${successCount} of ${displayedTransactions.length} payments recorded successfully.`,
      });
    }
    if (successCount !== displayedTransactions.length) {
      const failedCount = displayedTransactions.length - successCount;
      toast({
        title: "Batch Payment Issues",
        description: `${failedCount} payments could not be recorded.`,
        variant: "destructive",
      });
    }
    ...
    >
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Liability started on {format(new Date(liability.start_date), "PPP")}. Select a
            transaction from &apos;{accountName}&apos; to pre-fill payment details, or enter
            manually.
          </DialogDescription>
        </DialogHeader>
        ...
        {transactionsError && (
          <p className="text-red-500">Error loading transactions: {transactionsError.message}</p>
        )}

        {!isLoadingTransactions &&
          displayedTransactions.length === 0 &&
          allFetchedTransactions.length > 0 && (
            <p className="text-muted-foreground my-4">
              All suggested transactions have been processed. You can still record a manual payment
              or finish.
            </p>
          )}
        {!isLoadingTransactions && allFetchedTransactions.length === 0 && (
          <p className="text-muted-foreground my-4">
            No transactions found for &apos;{accountName}&apos; between{" "}
            {format(new Date(liability.start_date), "PPP")} and today. You can record a manual
            payment.
          </p>
        )}
          ...
          <div className="my-4">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-lg font-medium">
                Relevant Transactions from &apos;{accountName}&apos;
              </h3>
              <Button
                variant="outline"
                size="sm"
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              {form.formState.errors.payment_date && (
                <p className="text-red-500 text-sm">{form.formState.errors.payment_date.message}</p>
              )}
            </div>
            <div className="space-y-2">
          ...
          {selectedTransaction && (
            <p className="text-sm text-muted-foreground">
              Linked to transaction: &apos;{selectedTransaction.description}
              &apos; on {format(new Date(selectedTransaction.date), "yyyy-MM-dd")} for{" "}
          ...
          <DialogFooter className="pt-4">
            ...
            <Button
              ...
              disabled={
                ...
                isBatchRecording ||
                (!form.formState.isDirty && !selectedTransaction)
              }
            >
