...
import { Account, CategoryMetadata, Transaction } from "../../types";
...
export const TransactionForm = memo(function TransactionForm({
  ...
  useEffect(() => {
    if (isEditMode && transaction) {
      form.reset({
        ...
        to_amount: transaction.to_amount != null ? String(transaction.to_amount) : "",
        type: transaction.type,
        category: transaction.category,
        subcategory: transaction.subcategory || "",
        date: new Date(transaction.date).toISOString().split("T")[0],
        date_accountability: transaction.date_accountability
          ? new Date(transaction.date_accountability).toISOString().split("T")[0]
  ...
  const fromAccount = accounts.find((a: Account) => a.id === watchFromAccountId);
  const toAccount = accounts.find((a: Account) => a.id === watchToAccountId);
  const fromCurrency = (fromAccount?.currency ?? "EUR").toUpperCase();
  const toCurrency = (toAccount?.currency ?? "EUR").toUpperCase();
  const isCrossCurrencyTransfer = watchType === "transfer" && fromCurrency !== toCurrency;
  ...
  const subcategoryOptions: Option[] = useMemo(() => {
    ...
    const selectedCategory = (categories as unknown as CategoryMetadata[]).find(
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (isEditMode && transaction) {
      ...
      createMutation.mutate(
        {
          ...submitPayload,
          refunded_amount: submitPayload.refunded_amount ?? 0,
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        ...
        aria-describedby={
          isEditMode ? "edit-transaction-description" : "add-transaction-description"
        }
      >
        <DialogHeader>
          ...
          <div
            id={isEditMode ? "edit-transaction-description" : "add-transaction-description"}
            className="mt-2"
          >
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-base">
                    {isCrossCurrencyTransfer ? `Amount (${fromCurrency})` : "Amount"}
                  </FormLabel>
                  <FormControl>
                    <Input
                      className="h-12"
                      ...
                      step="0.01"
                      placeholder={
                        isCrossCurrencyTransfer ? `Amount in ${fromCurrency}` : "Enter amount"
                      }
                      {...field}
                    />
                  </FormControl>
            ...
            {isCrossCurrencyTransfer && (
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-base">Amount received ({toCurrency})</FormLabel>
                    <FormControl>
                      <Input
                        className="h-12"
                        type="number"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {(watchType === "expense" || (isEditMode && transaction?.type === "expense")) &&
