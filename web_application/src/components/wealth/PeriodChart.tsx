...
import { Area, AreaChart, Bar, BarChart, CartesianGrid, Legend, Line, ResponsiveContainer, Tooltip, TooltipProps, XAxis, YAxis } from "recharts"
...
export function PeriodChart({ data }: PeriodChartProps) {
  ...
  const calculateMetrics = (summaries: PeriodSummary[]): PeriodMetrics => {
    const sortedData = [...summaries].sort((a, b) => new Date(a.start_date).getTime() - new Date(b.start_date).getTime())
    ...
    return {
      ...
      savings: {
        ...
        average: savingsValues.reduce((sum, val) => sum + val, 0) / savingsValues.length,
        savingsRate: (savingsValues.reduce((sum, val) => sum + val, 0) /
          incomeValues.reduce((sum, val) => sum + val, 0)) * 100,
        bestPeriod: sortedData.reduce((best, current) => {
  ...
  const categoryTotals = data.summaries.reduce((totals, summary) => {
  ...
  const categoryColors = allCategories?.expense?.reduce((acc: Record<string, string>, category: any) => {
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className={`font-medium ${
                  metrics.income.growth.trend === 'up' ? 'text-success' :
                  metrics.income.growth.trend === 'down' ? 'text-destructive' : ''
                }`}>
              ...
              <div className="flex justify-between">
                ...
                <span className="text-sm">
                  ...
                  {' - '}
                  <span className="text-destructive">{formatCurrency(metrics.income.minValue)}</span>
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Growth</span>
                <span className={`font-medium ${
                  metrics.expense.growth.trend === 'down' ? 'text-success' :
                  metrics.expense.growth.trend === 'up' ? 'text-destructive' : ''
                }`}>
              ...
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Range</span>
                <span className="text-sm">
                  <span className="text-destructive">{formatCurrency(metrics.expense.maxValue)}</span>
                  {' - '}
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className={`font-medium ${
                  metrics.savings.savingsRate >= 20 ? 'text-success' :
                  metrics.savings.savingsRate >= 10 ? 'text-warning' :
                  'text-destructive'
                }`}>
