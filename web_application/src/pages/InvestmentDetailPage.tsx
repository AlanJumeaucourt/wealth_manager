...
import { ArrowDown, ArrowLeft, ArrowUp, Building2, DollarSign, LineChart, PieChart, Wallet } from "lucide-react"
...
export function InvestmentDetailPage() {
  ...
  const filterDataByPeriod = (data: typeof priceHistory) => {
    ...
    const sortedDataPoints = [...data].sort((a, b) =>
  ...
  const priceChange = latestPrice && previousPrice
    ...
    : 0
  const priceChangePercentage = latestPrice && previousPrice
  ...
  const formatDate = (date: string) => {
    const options: Intl.DateTimeFormatOptions = {
      month: 'short',
      day: 'numeric',
      year: selectedPeriod === "3Y" || selectedPeriod === "5Y" || selectedPeriod === "ALL" ? 'numeric' : undefined
    }
      ...
      <div className="space-y-6 p-6">
          ...
          <Card className="p-6 lg:col-span-2">
            ...
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={filteredPriceHistory || []} margin={{ top: 10, right: 10, bottom: 0, left: 10 }}>
                  <defs>
                  ...
                  <Tooltip
                    content={({ active, payload }) => {
                      ...
                      const dateTransactions = transactions?.filter(t =>
                        new Date(t.date).toISOString().split('T')[0] === new Date(data.date).toISOString().split('T')[0]
                      )
                          ...
                          {dateTransactions?.map((t, i) => (
                            <div key={i} className="border-t pt-2 mt-2 text-sm">
                              ...
                              <div className="text-muted-foreground">
                                {t.quantity} shares @ {new Intl.NumberFormat(undefined, {
                                  style: "currency",
                                  currency: assetInfo?.currency || "USD"
                                }).format(t.unit_price)}
                              </div>
                  ...
                  {transactions?.map((transaction) => {
                    ...
                    const closestDataPoint = filteredPriceHistory?.length
                      ? filteredPriceHistory.reduce((prev, curr) => {
                          ...
                          const prevDiff = Math.abs(prevDate.getTime() - transactionDate.getTime())
                          const currDiff = Math.abs(currDate.getTime() - transactionDate.getTime())
                    ...
                    return (
                      <ReferenceLine
                        ...
                        stroke={transaction.activity_type === "Buy" ? "hsl(var(--success))" : "hsl(var(--destructive))"}
                        strokeDasharray="3 3"
                        label={{
                          position: "top",
                          value: "●",
                          fill: transaction.activity_type === "Buy" ? "hsl(var(--success))" : "hsl(var(--destructive))",
                          fontSize: 24,
                          offset: 8
                        }}
                      />
              ...
              <div className="space-y-4">
                  ...
                  <>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium">
                        {(assetDetails.fund_profile[symbol]["Annual Holdings Turnover"] * 100).toFixed(2)}%
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground">Expense Ratio</span>
                      <span className="font-medium">
                        {(assetDetails.fund_profile[symbol]["Annual Report Expense Ratio"] * 100).toFixed(2)}%
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                ...
                {assetDetails?.fund_holding_info && (
                  <div className="pt-4 border-t">
                    <div className="text-sm font-medium text-muted-foreground mb-3">Asset Allocation</div>
                      ...
                      {Object.entries(assetDetails.fund_holding_info)
                        ...
                        .map(([key, value]) => (
                          <div key={key} className="space-y-1">
                            <div className="flex items-center justify-between">
                              <span className="capitalize">{key.replace(/([A-Z])/g, ' $1').trim().toLowerCase()}</span>
                  ...
                  {Object.entries(assetDetails.fund_top_holdings["Holding Percent"])
                    ...
                    .map(([symbol, percentage]) => (
                      <div key={symbol} className="flex items-center justify-between">
                        <div>
                          <div className="font-medium">{assetDetails?.fund_top_holdings?.["Name"]?.[symbol] ?? symbol}</div>
        ...
        <Tabs defaultValue="transactions" className="space-y-6">
          ...
          <TabsContent value="dividends">
            <Card className="p-6">
              <div className="space-y-6">
                {assetDetails?.dividends && Object.entries(assetDetails.dividends)
                  .sort((a, b) => new Date(b[0]).getTime() - new Date(a[0]).getTime())
                  .map(([date, amount]) => (
                    <div key={date} className="flex items-center gap-4 p-4 rounded-lg bg-muted">
                ...
                {(!assetDetails?.dividends || Object.keys(assetDetails.dividends).length === 0) && (
                  <div className="text-center py-6 text-muted-foreground">
                    No dividends found
                  </div>
                )}
