...
export default function LiabilitiesPage() {
  ...
  const [liabilityForInitialPayment, setLiabilityForInitialPayment] = useState<Liability | null>(
    null,
  );
  ...
  const filteredLiabilities =
    liabilities?.items?.filter((liability) => {
      const matchesSearch =
        searchTerm === "" ||
  ...
  const handleCreateLiability = (data: any) => {
    createLiability.mutate(data, {
      onSuccess: (createdLiability) => {
        const liability = createdLiability as Liability;
        setIsAddDialogOpen(false); // Close the main add dialog

        if (liability.account_id && liability.start_date) {
          const today = new Date();
          const liabilityStartDate = new Date(liability.start_date);
          ...
          if (liabilityStartDate < today) {
            ...
            setLiabilityForInitialPayment(liability);
  ...
  const handleViewLiabilityDetails = (liability: Liability) => {
    void router.navigate({ to: `/liabilities/${liability.id}` });
  };
      ...
      <Tabs defaultValue="dashboard">
        ...
        <TabsContent value="dashboard" className="mt-6">
          {liabilitiesData?.items && paymentsData?.items && (
            <LiabilityDashboard liabilities={liabilitiesData.items} payments={paymentsData.items} />
          )}
        </TabsContent>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the liability &quot;
              {liabilityToDelete?.name}&quot; and all associated payments. This action cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
