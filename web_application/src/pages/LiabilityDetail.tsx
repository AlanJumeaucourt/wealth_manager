...
import { AmortizationScheduleItem, LiabilityFormData, LiabilityPayment } from "@/types/liability";
...
export default function LiabilityDetailPage() {
  ...
  const { liabilityId } = useParams({
    from: "/authenticated/liabilities/$liabilityId",
  });
  ...
  const { data: amortizationSchedule, isLoading: isLoadingSchedule } =
    useLiabilityAmortization(liabilityId);
  ...
  const startDate = liability?.start_date
    ? format(parseISO(liability.start_date), "MMMM d, yyyy")
    : "N/A";
  const endDate = liability?.end_date
    ? format(parseISO(liability.end_date), "MMMM d, yyyy")
    : "N/A";
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-4">
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.navigate({ to: "/liabilities" })}
          >
            <ArrowLeftIcon className="h-4 w-4 mr-2" />
            Back
          </Button>
          ...
          <CardContent>
            ...
            {deferralInfo && deferralInfo.isCurrentlyInDeferral && (
              <div className="mt-2 text-xs bg-blue-50 text-blue-800 px-2 py-1 rounded-md">
                In {deferralInfo.deferralType} deferral until{" "}
                {deferralInfo.deferralFormattedEndDate}
              </div>
            )}
            {deferralInfo &&
              !deferralInfo.isCurrentlyInDeferral &&
              deferralInfo.deferralPeriodMonths > 0 && (
                <div className="mt-2 text-xs bg-gray-50 text-gray-600 px-2 py-1 rounded-md">
                  {deferralInfo.deferralType} deferral for {deferralInfo.deferralPeriodMonths}{" "}
                  months
                </div>
              )}
          </CardContent>
              ...
              {amortizationSchedule && amortizationSchedule.length > 0 ? (
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Detailed breakdown of payments over the loan term
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex items-center justify-center h-80">
